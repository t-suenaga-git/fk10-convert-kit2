    ...
    Public Class Rendo_sosin_Base_Repository
        ...
        Public Class SubConv
            ...
            Function Get_UseQry(ByRef sortstr As String) As String Implements IConv.Get_UseQry
                ...
                Dim rendoid As String = IIf(Njc.Frm.MainFrm.txtRendoID.Text = "", "kariID", Njc.Frm.MainFrm.txtRendoID.Text)  '20160825 連動ID入力箇所の新規追加 -add

                sortstr = "[送信設定順]"

                '抽出データの改行文字列を除去する暫定処理
                Dim tmp_midheader As String = "[送信設定順],[送信設定名],[掲載支店],[連動ID],[広告確認からの確認期間],[基準日],[基準日区分],[Eメールアドレス1],[Eメールアドレス2],[Eメールアドレス3],[Eメールアドレス4],[Eメールアドレス5]"
    ...
    Public Class Rendo_kokoku_jisyaweb_Repository
        ...
        Public Class SubConv
            ...
            Function Get_UseQry(ByRef sortstr As String) As String Implements IConv.Get_UseQry
                ...
                '抽出データの改行文字列を除去する暫定処理
                Dim tmp_midheader As String = "[物件No],[部屋No],[サイトNo],[セールスポイント補足],[担当者コメント],[特記事項],[広告備考],[天井高],[OAフロアの高さ],[賃料応相談]"
                Dim tmp_chgmidheader As String = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, True)
                '抽出データの改行文字列を除去する暫定処理
    ...
    Public Class Rendo_kokoku_athome_Repository
        ...
        Public Class SubConv
            ...
            Function Get_UseQry(ByRef sortstr As String) As String Implements IConv.Get_UseQry
                ...
                '抽出データの改行文字列を除去する暫定処理
                Dim tmp_midheader As String = "[物件No],[部屋No],[サイトNo],[プロのコメント],[プロのコメントスタッフID],[広告備考],[その他ペット可内容],[更新料区分],[保証金その他償却条件内容],[その他クレジット決済可],[その他クレジット決済可能条件等],[リフォーム有無],[リフォーム年月],[対象(内装関連)],[対象(外装関連)],[リフォーム内容],[リノベーション有無],[リノベーション実施年月],[リノベーション内容],[建物名表示],[部屋番号表示],[鍵現地対応],[賃料値下げ可],[初期費用値下げ可],[パノラマコンテンツID],[近隣駐輪場料金],[近隣駐輪場料金_税区分],[近隣バイク置き場料金],[近隣バイク置き場料金_税区分],[Yahoo公開不可]"
                Dim tmp_chgmidheader As String = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, True)
                '抽出データの改行文字列を除去する暫定処理
    ...
    Public Class Rendo_hyrui_Repository
        ...
        Public Class SubConv
            ...
            Function Get_UseQry(ByRef sortstr As String) As String Implements IConv.Get_UseQry
                ...
                '抽出データの改行文字列を除去する暫定処理
                Dim tmp_midheader As String = "[部屋分類No],[部屋分類名],[サイトNo],[サイト用部屋分類No]"
                Dim tmp_chgmidheader As String = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, True)
                '抽出データの改行文字列を除去する暫定処理
    ...
    Public Class Rendo_hysosin_Repository
        ...
        Public Class SubConv
            ...
            Function Get_UseQry(ByRef sortstr As String) As String Implements IConv.Get_UseQry
                ...
                '抽出データの改行文字列を除去する暫定処理
                Dim tmp_midheader As String = "[物件No],[部屋No],[ポータルサイトNo],[自社サービスNo],[選択No],[項目値]"
                Dim tmp_chgmidheader As String = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, True)
                '抽出データの改行文字列を除去する暫定処理
    ...
    Public Class Rendo_BtoBgroup_Repository
        ...
        Public Class SubConv
            ...
            Function Get_UseQry(ByRef sortstr As String) As String Implements IConv.Get_UseQry
                ...
                '抽出データの改行文字列を除去する暫定処理
                Dim tmp_midheader As String = "[物件No],[部屋No],[BtoBプラグイングループNo],[グループ公開有無]"
                Dim tmp_chgmidheader As String = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, True)
                '抽出データの改行文字列を除去する暫定処理
    ...
    Public Class Rendo_mapdisp_Repository
        ...
        Public Class SubConv
            ...
            Function Get_UseQry(ByRef sortstr As String) As String Implements IConv.Get_UseQry
                ...
                '抽出データの改行文字列を除去する暫定処理
                Dim tmp_midheader As String = "[物件No],[部屋No],[外部サイトNo（FK）],[自社サイトNo],[地図上に表示フラグ]"
                Dim tmp_chgmidheader As String = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, True)
                '抽出データの改行文字列を除去する暫定処理
