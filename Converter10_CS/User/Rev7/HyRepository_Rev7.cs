    ...
    public class Hy_mst_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                string tmp_midheader = "[物件NO],[部屋NO],[部屋分類名],[間取り],[間取り備考],[専有実面積(m2)],[専有実面積(坪)],[床面積(m2)],[床面積(坪)],[専有登記面積(m2)],[専有登記面積(坪)],[登記延床面積(m2)],[登記延床面積(坪)],[バルコニー面積(m2)],[バルコニー面積(坪)],[店舗付き住宅店舗部分面積(m2)],[店舗付き住宅店舗部分面積(坪)],[店舗付き住宅住宅部分面積(m2)],[店舗付き住宅住宅部分面積(坪)],[解約受付区分],[解約受付月数],[解約受付日数],[解約受付日にち],[自社Web内部キー採番ID]";
    ...
    public class Hy_mst_parking_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                // 抽出データの改行文字列を除去する暫定処理
                string tmp_midheader = "[物件NO],[部屋NO],[駐車場区分],[駐車場の空き数(手動設定用)],[駐車場の空き有無(手動設定用)],[駐車場料金区分(手動設定用)],[駐車場料金(手動設定用)],[駐車場料金税区分(手動設定用)],[駐車場の賃貸可能数]";
                string tmp_chgmidheader = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, true);
                // 抽出データの改行文字列を除去する暫定処理
    ...
    public class Hy_tokuyaku_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                string tmp_midheader = "[物件NO],[部屋NO],[特約区分],[内容1],[内容2],[内容3],[内容4],[内容5],[内容6],[内容7],[内容8],[内容9],[内容10],[内容11],[内容12],[内容13],[内容14],[内容15],[内容16],[内容17],[内容18],[内容19],[内容20],[内容21],[内容22],[内容23],[内容24],[内容25],[内容26],[内容27],[内容28],[内容29],[内容30],[内容31],[内容32],[内容33],[内容34],[内容35],[内容36],[内容37],[内容38],[内容39],[内容40],[内容41],[内容42],[内容43],[内容44],[内容45],[内容46],[内容47],[内容48],[内容49],[内容50]";
                // 抽出データの改行文字列を除去する暫定処理
    ...
    public class Hy_kagi_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                // 抽出データの改行文字列を除去する暫定処理
                string tmp_midheader = "[物件No],[部屋No],[鍵No],[鍵タイトル名],[鍵本数],[備考],[保管場所],[業者間での情報共有]";
                string tmp_chgmidheader = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, true);
                // 抽出データの改行文字列を除去する暫定処理
                ...
                switch (true)
                {
                    case object _ when My.MyProject.Forms.MainFrm.optHyKagi.Checked:  // 部屋鍵情報から取得する場合
                        {
                            // tmp_sql = tmp_sql & " SELECT * FROM "
                            tmp_sql = tmp_sql + " SELECT " + tmp_chgmidheader + " FROM ";
                            tmp_sql = tmp_sql + " ( ";
                    ...
                    case object _ when My.MyProject.Forms.MainFrm.optKyKagi.Checked:  // 契約鍵情報から取得する場合
                        {
                            // tmp_sql = tmp_sql & " SELECT * FROM "
                            tmp_sql = tmp_sql + " SELECT " + tmp_chgmidheader + " FROM ";
                            tmp_sql = tmp_sql + " ( ";
    ...
    public class Hy_mst_madriutiwake_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                // 抽出データの改行文字列を除去する暫定処理
                string tmp_midheader = "[物件NO],[部屋NO],[間取り内訳No],[間取り内訳区分],[畳数],[所在階],[既存間取内訳データ]";
                string tmp_chgmidheader = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, true);
                // 抽出データの改行文字列を除去する暫定処理
    ...
    public class Hy_mst_menseki_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                // 抽出データの改行文字列を除去する暫定処理
                string tmp_midheader = "[物件NO],[部屋NO],[面積タイプ],[面積No],[面積区分],[区画名],[面積(m2)],[坪数]";
                string tmp_chgmidheader = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, true);
                // 抽出データの改行文字列を除去する暫定処理
    ...
    public class Hy_setubi_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                // 抽出データの改行文字列を除去する暫定処理
                string tmp_midheader = "[物件NO],[部屋NO],[設備項目名],[設備内容]";
                string tmp_chgmidheader = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, true);
                // 抽出データの改行文字列を除去する暫定処理
    ...
    public class Hy_kanri_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                // 抽出データの改行文字列を除去する暫定処理
                string tmp_midheader = "[物件NO],[部屋NO],[月区分],[入金項目名],[入金項目行No],[入金項目区分],[請求額],[税区分],[算出区分],[算出基準入金項目名],[算出ヶ月],[請求先No],[請求月区分],[請求開始月],[請求パターン],[固定公共料金で使用],[請求発生年],[請求発生月],[備考]";
                string tmp_chgmidheader = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, true);
                // 抽出データの改行文字列を除去する暫定処理
    ...
    public class Hy_kanri_hendo_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                // 抽出データの改行文字列を除去する暫定処理
                string tmp_midheader = "[物件NO],[部屋NO],[行No],[メーター分類],[メーター名],[変動費入金項目名],[備考],[変動費請求ルールNo],[変動ルール備考],[請求月区分]";
                string tmp_chgmidheader = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, true);
                // 抽出データの改行文字列を除去する暫定処理
    ...
    public class Hy_szeniji_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                // 抽出データの改行文字列を除去する暫定処理
                string tmp_midheader = "[物件NO],[部屋NO],[修繕及び維持管理No],[修繕及び維持管理の箇所],[対象区分],[業者no],[氏名(商号または名称)(SJIS)],[住所(主たる事務所の所在地)],[連絡先電話番号],[自社no],[貸主No],[所有者No],[氏名(商号または名称)]";
                string tmp_chgmidheader = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, true);
                // 抽出データの改行文字列を除去する暫定処理
    ...
    public class Hy_biko_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                // 抽出データの改行文字列を除去する暫定処理
                string tmp_midheader = "[物件NO],[部屋NO],[メモ1],[メモ2],[メモ3],[メモ4],[メモ5],[メモ6],[メモ7],[メモ8],[メモ9],[メモ10],[メモ11],[メモ12],[メモ13],[メモ14],[メモ15],[メモ16],[メモ17],[メモ18],[メモ19],[メモ20],[メモ21],[メモ22],[メモ23],[メモ24],[メモ25],[メモ26],[メモ27],[メモ28],[メモ29],[メモ30],[メモ31],[メモ32],[メモ33],[メモ34],[メモ35],[メモ36],[メモ37],[メモ38],[メモ39],[メモ40],[メモ41],[メモ42],[メモ43],[メモ44],[メモ45],[メモ46],[メモ47],[メモ48],[メモ49],[メモ50]";
                string tmp_chgmidheader = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, true);
                // 抽出データの改行文字列を除去する暫定処理
