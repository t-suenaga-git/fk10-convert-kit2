    ...
    public class M_jisya_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                string tmp_midheader = "[自社・支店No],[自社・支店名],[自社・支店名（SJIS）],[自社・支店カナ],[自社・支店名２],[自社・支店名２（SJIS）],[自社・支店名３],[自社・支店名３（SJIS）],[代表者役職],[代表者名],[郵便番号],[住所1],[住所2],[TEL1],[TEL2],[携帯TEL1],[携帯TEL2],[FAX],[メールアドレス],[携帯メールアドレス],[Webアドレス],[備考（基本情報）],[免許証番号],[免許年月日],[取引主任者登録番号（代表）],[取引主任者名（代表）],[全国賃貸不動産管理業協会会員番号],[賃貸不動産経営管理士登録番号],[賃貸不動産経営管理士名],[賃貸住宅管理業者登録番号],[加入団体１],[加入団体２],[加入団体３],[加入団体４],[加入団体５],[加入団体６],[加入団体７],[加入団体８],[加入団体９],[加入団体１０],[代表者名（SJIS）],[取引主任者名（代表）（SJIS）],[賃貸不動産経営管理士名（SJIS）]";
    ...
    public class M_jisya_Koza_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                // 抽出データの改行文字列を除去する暫定処理
                string tmp_midheader = "[口座取得元],[各口座設定No],[各口座設定名称],[金融機関No],[金融機関店No],[口座種別],[口座番号],[口座名義],[口座名義カナ],[ゆうちょ口座記号１],[ゆうちょ口座記号２],[ゆうちょ口座番号],[備考(口座情報)]";
                string tmp_chgmidheader = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, true);
                // 抽出データの改行文字列を除去する暫定処理
    ...
    public class M_tanto_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                // 抽出データの改行文字列を除去する暫定処理
                string tmp_midheader = "[賃貸革命ログオンユーザNo],[賃貸革命ログオンユーザ名],[賃貸革命ログオンユーザ名(SJIS)],[賃貸革命ログオンユーザ名カナ],[賃貸革命ログオングループNo],[TEL1],[TEL2],[携帯電話1],[FAX1],[メールアドレス1],[携帯メールアドレス1],[宅建免許番号],[備考],[担当者ログオン禁止フラグ],[使用しないフラグ],[ログオンパスワード],[windowsログオンユーザー名],[意見画面表示フラグ]";
                string tmp_chgmidheader = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, true);
                // 抽出データの改行文字列を除去する暫定処理
    ...
    public class M_jisya_Biko_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                // 抽出データの改行文字列を除去する暫定処理
                string tmp_midheader = "[自社・支店No],[メモ1],[メモ2],[メモ3],[メモ4],[メモ5],[メモ6],[メモ7],[メモ8],[メモ9],[メモ10],[メモ11],[メモ12],[メモ13],[メモ14],[メモ15]";
                string tmp_chgmidheader = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, true);
                // 抽出データの改行文字列を除去する暫定処理
    ...
    public class M_furi_irai_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                // 抽出データの改行文字列を除去する暫定処理
                string tmp_midheader = "[振込依頼人No],[振込依頼人名],[金融機関No],[金融機関支店No],[口座種別],[口座番号],[口座名義],[振込依頼人コード],[振込依頼人カナ],[備考(基本情報)],[送信ファイルパス],[ヲ変換フラグ(0:変換しない 1:変換する)],[負担調整フラグ(0:調整しない 1:調整する)],[総合振込データ生成フラグ]";
                string tmp_chgmidheader = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, true);
                // 抽出データの改行文字列を除去する暫定処理
    ...
    public class M_furi_tesu_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                // 抽出データの改行文字列を除去する暫定処理
                string tmp_midheader = "[振込依頼人No],[レコードNo],[金額From],[金額To],[同行同支店手数料(電信扱)],[同行同支店手数料(文書扱)],[同行他支店手数料(電信扱)],[同行他支店手数料(文書扱)],[他行手数料(電信扱)],[他行手数料(文書扱)]";
                string tmp_chgmidheader = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, true);
                // 抽出データの改行文字列を除去する暫定処理
    ...
    public class M_ns_syutoku_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                // 抽出データの改行文字列を除去する暫定処理
                string tmp_midheader = "[入出金取得No],[設定名称],[設定カナ名称],[フォーマット定義名],[受信ファイル名（フルパス）],[取込データ種別],[備考],[家賃口座No],[送信ファイル名（フルパス）],[預金分割設定],[重複チェック]";
                string tmp_chgmidheader = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, true);
                // 抽出データの改行文字列を除去する暫定処理
    ...
    public class Mspc_area_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                // 抽出データの改行文字列を除去する暫定処理
                string tmp_midheader = "[SPCエリアNo],[SPCエリア名称],[備考]";
                string tmp_chgmidheader = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, true);
                // 抽出データの改行文字列を除去する暫定処理
    ...
    public class Mspc_accpoint_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                // 抽出データの改行文字列を除去する暫定処理
                string tmp_midheader = "[SPC地区No],[SPCエリアNo],[SPC地区名称],[備考]";
                string tmp_chgmidheader = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, true);
                // 抽出データの改行文字列を除去する暫定処理
    ...
    public class Mspc_setuzoku_Repository
    {
        ...
        public class SubConv : IConv
        {
            ...
            public string Get_UseQry(ref string sortstr)
            {
                ...
                // 抽出データの改行文字列を除去する暫定処理
                string tmp_midheader = "[接続方法No],[設定名],[回線種別(1:電話回線 2:ISDN)],[開発タイプ],[開発名],[接続方法(1:アクセスポイント 2:TEL)],[エリアNo],[地区No],[電話番号],[外線No],[リトライ回数],[リトライの間隔],[利用するサービスコード],[備考],[改行有無]";
                string tmp_chgmidheader = EtcMethod.Get_FldnameAddReplaceMethod(tmp_midheader, true);
                // 抽出データの改行文字列を除去する暫定処理
